...
def create_inventory_item_asset_instance(logger, data, status_cache, item_cache):
    """
    Crea una instancia de SenitronItemAsset a partir de los datos proporcionados.
    
    Args:
        logger: Objeto logger para registrar errores.
        data (dict): Datos del activo.
        status_cache (dict): Cache para objetos SenitronStatus.
        item_cache (dict): Cache para objetos SenitronItem.
    ...
    try:
        ...
        if status_key in status_cache:
            ...
            status, _ = SenitronStatus.objects.update_or_create(
                name=status_json.get('name'),
                senitron_id=status_json.get('id')
            )
